                     ...
                     SeafDirent *newdent)
{
    ...
    for (ptr = olddir->entries; ptr; ptr = ptr->next) {
        ...
        id = post_file_recursive (repo, dent->id, remain, newdent);
        if (id != NULL) {
              ...
              SeafDirent *dent)
{
    ...
    return post_file_recursive(repo, root_id, parent_dir, dent);
}
                            ...
                            GError **error)
{
    ...
    if (!check_file_exists (repo->store_id, repo->version, head_commit->root_id,
                            canon_path, file_name, NULL)) {
        if (!create) {
            g_set_error (error, SEAFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                         "file does not exist");
            ret = -1;
            ...
            return seaf_repo_manager_post_file (mgr, repo_id, temp_file_path,
                                                parent_dir, file_name, user,
                                                new_file_id, error);
        }
    }
                                   ...
                                   GError **error)
{
    ...
    if (!check_file_exists (repo->store_id, repo->version, head_commit->root_id,
        ...
        if (!create) {
            ...
            return seaf_repo_manager_post_file_blocks (mgr, repo_id, parent_dir,
                                                       file_name, blockids_json,
                                                       paths_json, user, file_size,
                                                       new_file_id, error);
        }
    }
...
static char *
get_file_id_with_cache (SeafRepo *repo,
                        SeafCommit *commit,
                        const char *path,
                        GHashTable *file_id_cache,
                        GError **error)
